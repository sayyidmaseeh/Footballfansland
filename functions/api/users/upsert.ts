...
export async function onRequest(context: any) {
  ...
  const { username, email, favoriteClub, isAdmin, picture, freeSlots } = body;
  if (!email) {
    return new Response(JSON.stringify({ success: false, error: "Missing user email key" }), {
      status: 400,
      headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
    });
  }
  ...
  try {
    ...
    const { error } = await supabase.from("users").upsert({
      ...
      favorite_club: favoriteClub || "Argentina",
      is_admin: isAdmin !== undefined ? !!isAdmin : false,
      picture: picture || "",
      free_slots: freeSlots !== undefined ? freeSlots : 3
    }, {
      onConflict: "email"
    });
